...
class DivideConquer {
 public:
  ...
  virtual Solution* solve(const Instance* I) const = 0;
  Solution* solve(const Instance* I, const int& size) const {
    if (small_->small(I)) 
  ...
  void setStrategy(Combine* a, Divide* b, Small* c, SolveSmall* d) {
    combine_ = a;
    divide_ = b;
    small_ = c;
    solve_small_ = d;
